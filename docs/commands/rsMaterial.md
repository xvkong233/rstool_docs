...
**备注**：新版窗口不可用时自动回退到 rsMaterialLibraryOld 旧版。索引以根目录为粒度，缩略图与贴图缓存到本地；赋予走 Rhino 材质 API，开启自动贴图轴时会按当前类型+尺寸自动加 Surface/BoxMapping。

## 材质库准备

rsMaterial 使用本地材质库，用户需要自行整理材质文件及相关贴图，并将材质资产归类存放到选定的库目录中。

如需领取 RSTool 官方整理的材质库，可将 RSTool 相关视频转发到朋友圈，保留转发截图并联系 RSTool 客服领取。
