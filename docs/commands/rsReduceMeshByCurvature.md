...
![按曲率减面输入：原始网格灰度渲染（密集三角面，22% 黑色为谷地阴影）](../assets/rsReduceMeshByCurvature/image1.png)
*按曲率减面的输入（Before）：原始网格的灰度渲染（无 UI 面板），保留了大尺度的曲面起伏；密集三角面在谷底与转角投下深黑阴影（约 22% 黑色像素集中在低洼与折角处），整体仍清晰呈现原模型的尖锐轮廓；下一张即对该网格执行按曲率减面后的结果*

![按曲率减面结果：黄色减面后网格 + 参数对话框（2 593 920 → 74 080 面，减少 97.1%）](../assets/rsReduceMeshByCurvature/image2.png)
*按曲率减面的结果（After）：Rhino 视口，原始曲面上以**黄色线框**叠加显示减面后的网格（密集三角面 → 稀疏三角面）；右侧是本命令的「按曲率减面」参数对话框（已应用）：预设（自定义/保守/均衡/强力）、最小边长 4.0、最大边长 100、最小角 25°、曲率偏置 1.00、迭代次数 12、保护裸边/角点/硬折边、隐藏原始网格等参数；底部实时显示面数变化（2 593 920 → 74 080 面，减少 97.1%）*
